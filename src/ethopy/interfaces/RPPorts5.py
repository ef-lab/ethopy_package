...
class RPPorts5(Interface):
    def __init__(self, **kwargs):
        if not globals()["IMPORT_RP"]:
            raise ImportError(
                "Could not import RPi packages (rpi-lgpio)! "
                "Please install: pip install rpi-lgpio"
            )
        super(RPPorts5, self).__init__(**kwargs)
        self.GPIO = GPIO
        self.GPIO.setmode(self.GPIO.BCM)

        # Store the lgpio chip handle for tx_pulse
        self.chip = GPIO._chip  # Access the internal chip handle
        ...
        self._sound_pwm = None
        self._sound_stop_event = threading.Event()

        matched_ports = set(self.rew_ports) & set(self.channels["Liquid"].keys())
        assert matched_ports == set(self.rew_ports), (
            "All reward ports must have assigned a liquid delivery port!"
        )

        if "Lick" in self.channels:
            self.GPIO.setup(
                list(self.channels["Lick"].values()),
                self.GPIO.IN,
            ...
            if self.callbacks:
                for channel in self.channels["Lick"]:
                    self.GPIO.add_event_detect(
                        self.channels["Lick"][channel],
                        self.GPIO.RISING,
                        callback=self._lick_port_activated,
                        bouncetime=100,
                    )

        if "Proximity" in self.channels:
            self.GPIO.setup(
                list(self.channels["Proximity"].values()),
                self.GPIO.IN,
                pull_up_down=GPIO.PUD_DOWN,
            )
            if self.callbacks:
                for channel in self.channels["Proximity"]:
                    self.GPIO.add_event_detect(
                        self.channels["Proximity"][channel],
                        self.GPIO.BOTH,
                        callback=self._position_change,
                        bouncetime=50,
                    )

        if "Odor" in self.channels:
            self.GPIO.setup(
                list(self.channels["Odor"].values()),
                self.GPIO.OUT,
        ...
        if "Liquid" in self.channels:
            for channel in self.channels["Liquid"]:
                # Set up liquid delivery pins using RPi.GPIO
                self.GPIO.setup(
                    self.channels["Liquid"][channel],
                    self.GPIO.OUT,
                    initial=self.GPIO.LOW,
                )
        ...
        if "Sound" in self.channels:
            for channel in self.channels["Sound"]:
                # Set up sound pins using RPi.GPIO
                self.GPIO.setup(
                    self.channels["Sound"][channel],
                    self.GPIO.OUT,
                    initial=self.GPIO.LOW,
                )
        ...
        if "Status" in self.channels:
            self.GPIO.setup(
                self.channels["Status"], self.GPIO.OUT, initial=self.GPIO.LOW
            )

        if "Sync" in self.channels and "out" in self.channels["Sync"]:
            self.GPIO.setup(
                self.channels["Sync"]["out"], self.GPIO.OUT, initial=self.GPIO.LOW
            )

        if self.exp.sync:
            self.GPIO.setup(
                self.channels["Sync"]["rec"], self.GPIO.IN, pull_up_down=GPIO.PUD_DOWN
            )
            self.GPIO.setup(
                self.channels["Sync"]["in"], self.GPIO.IN, pull_up_down=GPIO.PUD_DOWN
            )
            self.GPIO.add_event_detect(
                self.channels["Sync"]["in"],
                self.GPIO.BOTH,
                callback=self._sync_in,
                bouncetime=20,
            )
            self.dataset = self.logger.createDataset(
                dataset_name="sync", dataset_type=np.dtype([("sync_times", np.double)])
            )
    ...
    def opto_stim(self, duration, dutycycle):
        self.thread.submit(self._pwd_out, self.channels["Opto"], duration, dutycycle)
    ...
    def give_sound(self, sound_freq=40500, volume=100, pulse_freq=0):
        self.thread.submit(
            self.__pulse_out, self.channels["Sound"][1], sound_freq, volume, pulse_freq
        )

    def stop_sound(self):
        # Signal the sound generation thread to stop
        self._sound_stop_event.set()
        # Stop PWM if it exists
        if self._sound_pwm:
            self._sound_pwm.stop()
            self._sound_pwm = None
        # Ensure the sound pin is off
        if "Sound" in self.channels:
            self.GPIO.output(self.channels["Sound"][1], self.GPIO.LOW)
    ...
    def _position_change(self, channel=0):
        ...
        Also called from function in_position in the case where the callback has not run but the position has changed.
    ...
    def _give_pulse(self, port, duration):
        """Generate liquid reward pulse using lgpio.tx_pulse for precise timing."""
        pin = self.channels["Liquid"][port]

        # Convert milliseconds to microseconds
        pulse_us = int(duration * 1000)

        # Generate a single hardware-timed pulse
        # tx_pulse(chip, gpio, pulse_on_us, pulse_off_us, offset_us, cycles)
        lgpio.tx_pulse(self.chip, pin, pulse_us, 0, 0, 1)
    ...
    def _pwd_out(self, channel, duration, dutycycle):
        """PWM output using RPi.GPIO (now rpi-lgpio)."""
    ...
    def __pulse_out(self, channel, freq, dutycycle=100, pulse_freq=0):
        """Generate sound using RPi.GPIO.PWM instead of pigpio wave generation."""
        self._sound_stop_event.clear()

        if dutycycle == 0:
            return

        # Speaker has non-monotonic response with ~50% duty cycle is maximum response
        # Normalize percentage by /2 for speaker response
        normalized_duty = dutycycle / 2

        try:
            # Create PWM instance
            self._sound_pwm = self.GPIO.PWM(channel, freq)

            if pulse_freq == 0:
                # Continuous tone
                self._sound_pwm.start(normalized_duty)
                # Keep running until stop is requested
                while not self._sound_stop_event.is_set():
                    sleep(0.01)  # Small sleep to prevent CPU spinning
            else:
                # Pulsed tone - alternate between sound and silence
                pulse_period = 1.0 / (pulse_freq * 2)  # Time for one pulse (on + off)

                while not self._sound_stop_event.is_set():
                    # Turn on sound
                    self._sound_pwm.start(normalized_duty)
                    if self._sound_stop_event.wait(timeout=pulse_period):
                        break

                    # Turn off sound (silence period)
                    self._sound_pwm.stop()
                    if self._sound_stop_event.wait(timeout=pulse_period):
                        break

        except Exception as e:
            log.error(f"Error in sound generation: {e}")
        finally:
            # Clean up PWM
            if self._sound_pwm:
                self._sound_pwm.stop()
